...
/*When the page is loaded, we assemble all of the 
track HTML elements and their corresponding TextTrack objects*/
...
/*The buildingChapter navigation menu. All is done 
in the window.onload callback, so nothing happens until the DOM is ready*/
...
/*
the displayChapters(track) function will iterate over all of the cies 
within the chapter track passed as its parameter. For each cue,
the JSON content is re-formatted back into a JavaScript object (line72)
and the image filename and description of the chapter/cue are extracted (line 73-74).
Then an HTML description fot the chapter is built and added to the div element with 
id = chapterMenu.
*/
...
function displayChapterMarkersAfterTrackLoaded(trackElem, track) {
  // Create a listener that will be called only when the track has been loaded
